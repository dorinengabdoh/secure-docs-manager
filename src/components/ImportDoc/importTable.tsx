...
export const ImportTable: React.FC<ImportTableProps> = ({ isDark, onView }) => {
  ...
  const handleSubmitArchive = async (updatedArchive: NewArchive) => {
    ...
    try {
      ...
      // Show success alert
      showAlert(t.archiveUpdated || "Archive updated successfully", "archive");
      ...
      // Show error alert
      showAlert(
        error.response?.data?.error || "Failed to update archive.",
        "error"
      );
  ...
  return (
    ...
    >
      ...
      ) : filteredArchives.filter(
          (archive) =>
            archive.status === "draft" || archive.status === "rejected"
        ).length === 0 ? (
        <div className="text-center py-10 text-gray-500">
